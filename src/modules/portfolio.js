...
import { refreshIcons } from './renderers.js';

/**
 * Portfolio Renderer for Ekalavya (Supabase Implementation)
 */
export const renderPortfolio = async (id) => {
    const container = document.getElementById('main-content');
    if (!container) return;

    // Loading State
    container.innerHTML = `
        <div class="flex-center min-h-[60vh]">
            <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-ekalavya-gold"></div>
        </div>
    `;
    ...
    try {
                ...
                <div class="glass-card p-10 rounded-3xl mb-12 flex flex-col items-center text-center relative overflow-hidden">
                    ...
                    <div class="relative mb-6">
                        <div class="w-28 h-28 rounded-full border-2 border-ekalavya-gold/30 p-1">
                            <img src="${profile.photoURL}" alt="${profile.displayName}" class="w-full h-full rounded-full object-cover">
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-6 w-full max-w-md">
                        <div class="glass-card bg-white/5 p-6 rounded-2xl border border-white/10">
                            <div class="flex items-center justify-center gap-2 mb-2">
                                ...
                                <span class="text-3xl font-bold text-white counter-increment">${profile.score}</span>
                            </div>
                        ...
                        <div class="glass-card bg-white/5 p-6 rounded-2xl border border-white/10">
                            <div class="flex items-center justify-center gap-2 mb-2">
                                ...
                                <span class="text-3xl font-bold text-white">${profile.streak}</span>
                            </div>
        ...
        container.innerHTML = `<div class="text-white text-center py-20">Error loading portfolio</div>`;
    }
};
